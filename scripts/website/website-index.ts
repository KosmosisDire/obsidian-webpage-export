...
export interface WebpageData extends FileInfo
{
	title: string;
	icon: string;
	description: string;
...
export interface WebsiteData extends FileInfo
{
	webpages: {[path: string]: WebpageData},
...
export class WebsiteIndex
{
	...
	public previousMetadata: WebsiteData | undefined = undefined;
	public newMetadata: WebsiteData | undefined = undefined;
	...
	public async init(): Promise<boolean>
	{
		...
		if (this.isVersionChanged()) this.previousMetadata = undefined;
	...
	public shouldApplyIncrementalExport(printWarning: boolean = false): boolean
	{
		...
		let customHeadChanged = this.previousMetadata && (this.previousMetadata?.addCustomHead != (Settings.customHeadContentPath != ""));
		...
		let customFaviconChanged = this.previousMetadata && (this.previousMetadata?.addFavicon != (Settings.faviconPath != ""));
	...
	public async createIndex(): Promise<Asset | undefined>
	{
		...
		for (const webpage of htmlWebpages) 
		{
			...
			ExportLog.progress(progressCount, totalCount, "Indexing", "Adding: " + webpage.relativePath.stringify, "var(--color-blue)");

			const content = preprocessContent(webpage.viewElement);

			if (content) 
			{
				const webpagePath = webpage.relativePath.unixified().stringify;
		...
		for (const file of this.web.dependencies)
		{
			...
			const filePath = file.relativePath.stringify;
	...
	public async createMetadata(options: MarkdownWebpageRendererAPIOptions): Promise<Asset | undefined>
	{
		...
		this.newMetadata = Object.assign({}, JSON.parse(JSON.stringify(this.previousMetadata ?? "")) ?? {}) as WebsiteData;
		if (!this.newMetadata.webpages) this.newMetadata.webpages = {};
		if (!this.newMetadata.fileInfo) this.newMetadata.fileInfo = {};
		this.newMetadata.siteName = this.web.exportOptions.siteName ?? app.vault.getName();
		this.newMetadata.exportTime = this.exportTime;
		this.newMetadata.pluginVersion = HTMLExportPlugin.plugin.manifest.version;
		this.newMetadata.bodyClasses = Website.bodyClasses;
		this.newMetadata.addCustomHead = Settings.customHeadContentPath != "";
		this.newMetadata.addFavicon = Settings.faviconPath != "";
		this.newMetadata.attachedFiles = this.allFiles;
		this.newMetadata.dependencies = AssetHandler.getDownloads(options).map((asset) => asset.relativePath.unixified().stringify);
		...
		{
			if(MarkdownRendererAPI.checkCancelled()) return undefined;
			ExportLog.progress(progressCount, totalCount, "Creating Metadata", "Adding: " + page.relativePath.stringify, "var(--color-cyan)");

			let webpageInfo: WebpageData = {} as WebpageData;

			webpageInfo.title = page.title;
			webpageInfo.icon = page.icon;
			webpageInfo.description = page.descriptionOrShortenedContent;
			webpageInfo.aliases = page.aliases;
			webpageInfo.tags = page.tags;
			webpageInfo.headers = await page.getStrippedHeadings();
			webpageInfo.backlinks = page.backlinks.map((link) => link.relativePath.unixified().stringify);
			webpageInfo.author = page.author;
			webpageInfo.coverImageURL = "";
			webpageInfo.fullURL = page.fullURL;
			webpageInfo.pathToRoot = page.pathToRoot.unixified().stringify;

			webpageInfo.createdTime = page.source.stat.ctime;
			webpageInfo.modifiedTime = page.source.stat.mtime;
			webpageInfo.sourceSize = page.source.stat.size;
			webpageInfo.sourcePath = new Path(page.source.path).unixify().stringify;
			webpageInfo.exportPath = page.relativePath.unixified().stringify;
			webpageInfo.attachedFiles = page.dependencies.map((download) => download.relativePath.unixified().stringify);
			...
			fileInfo.attachedFiles = webpageInfo.attachedFiles;

			this.newMetadata.fileInfo[webpageInfo.sourcePath] = fileInfo;
			this.newMetadata.webpages[webpageInfo.sourcePath] = webpageInfo;
			progressCount++;
		}
		...
		{
			if(MarkdownRendererAPI.checkCancelled()) return undefined;
			ExportLog.progress(progressCount, totalCount, "Creating Metadata", "Adding: " + file.relativePath.stringify, "var(--color-cyan)");

			let fileInfo: FileInfo = {} as FileInfo;
			fileInfo.createdTime = this.exportTime;
			fileInfo.modifiedTime = this.exportTime;
			fileInfo.sourceSize = file.content.length;
			fileInfo.exportPath = file.relativePath.unixified().stringify;
			fileInfo.attachedFiles = [];

			let exportPath = file.relativePath.unixified().stringify;
			this.newMetadata.fileInfo[exportPath] = fileInfo;
			progressCount++;
		}
		...
		for (const oldFile of this.removedFiles)
		{
			...
			delete this.newMetadata.fileInfo[oldFile];
			progressCount++;
		}

		return new Asset("metadata.json", JSON.stringify(this.newMetadata, null, 2), AssetType.Other, InlinePolicy.Download, false, Mutability.Temporary);
	}
	...
	public async deleteOldFiles(options?: MarkdownWebpageRendererAPIOptions)
	{
		...
		for (let i = 0; i < this.removedFiles.length; i++)
		{
			...
			let removedFileInfo = this.previousMetadata.fileInfo[removedPath];
			if (!removedFileInfo) continue;
			
			let deletePath = this.web.destination.joinString(removedFileInfo.exportPath);
			console.log("Deleting old file: " + deletePath.stringify);
			await deletePath.delete();
			ExportLog.progress(i, this.removedFiles.length, "Deleting Old Files", "Deleting: " + deletePath.stringify, "var(--color-orange)");
		}
		...
		for	(let i = 0; i < folders.length; i++)
		{
			...
			ExportLog.progress(i, folders.length, "Deleting Empty Folders", "Deleting: " + folder.stringify, "var(--color-orange)");
	...
	{
		if (!this.previousMetadata) return;
		if (this.previousMetadata.bodyClasses == Website.bodyClasses) return;
		console.log("Updating body classes of previous export");

		let convertableFiles = this.previousMetadata.attachedFiles.filter((path) => MarkdownRendererAPI.isConvertable(path.split(".").pop() ?? ""));
		let exportedPaths = convertableFiles.map((path) => new Path(this.previousMetadata?.fileInfo[path]?.exportPath ?? "", this.web.destination.stringify));
		...
		for (let i = 0; i < exportedPaths.length; i++)
		{
			...
			let content = await exportedPath.readAsString();
			if (!content) continue;
			let dom = new DOMParser().parseFromString(content, "text/html");
			let body = dom.querySelector("body");
			if (!body) continue;
			body.className = Website.bodyClasses;
			await exportedPath.write(dom.documentElement.outerHTML);
			ExportLog.progress(i, exportedPaths.length, "Updating Body Classes", "Updating: " + exportedPath.stringify, "var(--color-yellow)");
		}
	}
	...
	public getMetadataForFile(file: TFile): FileInfo | undefined
	{
		return this.previousMetadata?.fileInfo[file.path];
	}

	public getMetadataForPath(path: string):  FileInfo | undefined
	{
	...
	public getAllFiles(): string[]
	{
		...
		{
			if (this.allFiles.some((path) => Path.equal(path, asset.relativePath.stringify))) continue;

			this.allFiles.push(asset.relativePath.unixified().stringify);
		}
	...
	public getRemovedFiles(): string[]
	{
		...
		this.removedFiles = this.previousMetadata.attachedFiles.filter((path) => 
		{
	...
	public getAddedFiles(): string[]
	{
		...
		this.addedFiles = this.allFiles.filter(path => !this.previousMetadata?.attachedFiles.includes(path));
	...
	public getKeptDependencies(): string[]
	{
		...
		{
			let dep = this.previousMetadata.fileInfo[file]?.attachedFiles ?? [];
