...
async function initGlobalObjects()
{
	...
	webpageContainer = document.querySelector("#layout");
	documentContainer = document.querySelector("#center-content");
	leftSidebar = document.querySelector("#left-sidebar");
	rightSidebar = document.querySelector("#right-sidebar");

	fileTree = document.querySelector("#file-explorer");
	outlineTree = document.querySelector(".outline-tree");
	fileTreeItems = Array.from(document.querySelectorAll(".tree-container#file-explorer .tree-item"));
...
async function loadDocument(url, changeURL, showInTree)
{
	...
	if (response.ok)
	{
		...
		if(extention == "html")
		{
			...
			let newDocumentEl = transferDocument.querySelector("#center-content");
		...
		else
		{
			...
			if(embedType != undefined)
			{
				...
				viewContent.setAttribute("class", "embed");
...
function setActiveDocument(url, showInTree, changeURL, animate = true)
{
	...
	let oldActiveTreeItem = document.querySelector("#file-explorer .tree-item > .tree-item-self.is-active");
	let newActiveTreeItem = document.querySelector(`#file-explorer .tree-item > .tree-item-self[href^="${searchlessHeaderlessPath}"]`);
...
function setupTrees(setupOnNode) 
{
	...
	{
		button.treeRoot = button.closest(".leaf-content").querySelector(".tree-container");
		...
		{ 
			let treeItems = button.treeRoot.id == "file-explorer" ? fileTreeItems : outlineTreeItems;
	...
	let fileTreeClick = Array.from(setupOnNode.querySelectorAll(".tree-container#file-explorer .tree-item.mod-collapsible > .tree-item-self"));
...
async function setTreeCollapsed(element, collapsed, animate = true, openParents = true)
{
	...
	else
	{
		...
		if (treeContainer)
		{
			let collapseButton = treeContainer.closest(".leaf-content").querySelector(".tree-collapse-all");
...
{
	return document.querySelector(`#file-explorer .tree-item:has(> .tree-item-self[href^="${path}"])`);
}
...
async function filterFileTree(showPathList, hintLabelLists, query, openFileTree = true)
{
	...
	let allItems = Array.from(document.querySelectorAll("#file-explorer .tree-item:not(.filtered-out)"));
...
async function clearFileTreeFilter(closeFileTree = true)
{
	...
	let filteredItems = document.querySelectorAll("#file-explorer .filtered-out");
	for await (let item of filteredItems)
	{
		item.classList.remove("filtered-out");
	}

	let markItems = document.querySelectorAll("#file-explorer .tree-item-self[href*='?mark=']");
...
{
	let treeItems = Array.from(document.querySelectorAll("#file-explorer .tree-item.nav-file:not(.filtered-out)"));
	...
	let folders = Array.from(document.querySelectorAll("#file-explorer .tree-item.nav-folder:not(.filtered-out)"));
...
{
	let leftHandle = document.querySelector('#left-sidebar .sidebar-handle');
	let rightHandle = document.querySelector('#right-sidebar .sidebar-handle');
...
async function search(query)
{
	...
	if (query.length >= 1)
	{
		...
		let fileTree = document.querySelector("#file-explorer");
		if (fileTree)
		{
