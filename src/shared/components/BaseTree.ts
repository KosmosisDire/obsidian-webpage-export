...
export class Tree
{
	...
	public startItemsCollapsed: boolean = false;
	...
	protected async generateTree(container: HTMLElement)
	{
		...
		{
			await this.buildTreeRecursive(item, container, this.minDepth, this.startItemsCollapsed);
		}
	...
	public async generate(container?: HTMLElement): Promise<HTMLElement>
	{
		...
		wrapper.id = this.id;
		wrapper.className = this.class + " tree-container";
		...
		if (this.title || this.addCollapseAllButton)
		{
			...
			if (this.addCollapseAllButton)
			{
				...
				collapseAllEl.className = "clickable-icon nav-action-button tree-collapse-all";
				header.appendChild(collapseAllEl);
					...
					if (this.startItemsCollapsed) collapseAllEl.classList.add("is-collapsed");
			}
		}
...
export class TreeItem
{
	...
	{
		const itemContentsEl = document.createElement('div');
		itemContentsEl.className = "tree-item-inner";
		container.appendChild(itemContentsEl);
		
		// TODO: Fix rendering titles
		// if (this.tree.renderMarkdownTitles) MarkdownRendererAPI.renderMarkdownSimpleEl(this.title, itemContentsEl);
		itemContentsEl.innerText = this.title;
	...
	protected async insertSelf(container: HTMLElement): Promise<HTMLElement>
	{
		...
		const itemLinkEl = document.createElement(this.href ? "a" : "div");
		itemLinkEl.className = "tree-item-self is-clickable";
		container.appendChild(itemLinkEl);
	...
	protected insertCollapseIcon(container: HTMLElement): HTMLElement | undefined
	{
		...
		this.collapseIcon.className = "tree-item-icon collapse-icon";
		container.appendChild(this.collapseIcon);
	...
	protected insertIcon(container: HTMLElement): HTMLDivElement | undefined
	{
		...
		itemIconEl.className = "tree-icon iconize-icon";
		container.appendChild(itemIconEl);

		// TODO: Fix rendering icons
	...
	protected insertChildren(container: HTMLElement): HTMLDivElement
	{
		...
		this.childContainer.className = "tree-item-children";
