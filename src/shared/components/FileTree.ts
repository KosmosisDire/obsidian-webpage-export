...
import { Tree, TreeItem } from "./BaseTree"; // Adjusted import if necessary
...
export class FileTree extends Tree
{
	...
	public constructor(files: string[] | Path[], sort = true)
	{
		super();

		// Convert string paths to Path objects if needed
		this.files = files.map(file => typeof file === 'string' ? new Path(file) : file);

		// make sure every path is a file
		if (this.files.some((file) => file.isDirectory))
		{
		...
		for (const file of filteredFiles)
		{
			...
			for (let i = 0; i < pathSections.length; i++)
			{
				...
				if (child == undefined)
				{
					...
					if(child.isFolder) 
					{
						...
						if (section && !section.isEmpty) // Check if it's a valid folder (not vault root)
						{
                            child.title = section.basename; // Use actual folder name
			...
			if (currentParentNode instanceof FileTreeItem && currentParentNode.dataRef === file.path)
			{
				...
				if (file.isDirectory) 
                {
                    ...
                    currentParentNode.title = file.basename; // Ensure folder title is correct
                }
				else 
				{
